...
if (chain1.length > chain2.length) {
  ...
  for (i in sort(unique(contact.residues$atom2))) {
    amino <- strsplit(contacts.map$atom2[match(i, contact.residues$atom2)], ' ')[[1]][1]
  ...
  for (i in sort(unique(contact.residues$atom1))) {
    amino <- strsplit(contacts.map$atom2[match(i, contact.residues$atom1)], ' ')[[1]][1]
    peptide.chain <- paste0(peptide.chain, i, "\n", amino, " ")
  }
}
...
# Plot all graph
out.name <- paste0(out.path, name, "_contact_map_all.pdf")
...
# Plot graph
for (i in seq_along(contact.hits)) {
  pdf.name <- paste0("_contact_map_", i - 1, "-", i, ".pdf")
